/*------------------------------------------------------------------------------*/
/* Description: 		Driver_UART.c contains functions managing UART*/
/*------------------------------------------------------------------------------*/
#include <msp430.h>
#include <string.h>
...
void envoi_msg_UART(char *msg[]) {
	...
	for (i = 0; msg[i] != 0x00; i++) {
		while ((IFG2 & UCA0TXIFG)!=(unsigned)0){	/*attente de fin du dernier envoi (UCA0TXIFG ?1 quand UCA0TXBUF vide)*/
			UCA0TXBUF = msg[i];
		}
	}
}
...
void interpreteur(void) {
	if (strcmp((const char *) cmd, "h") == 0) /*----------------------------------- help*/
			{
		envoi_msg_UART("\r\nCommandes :");
		envoi_msg_UART("\r\n'0' : demarrer moteur infra");
		envoi_msg_UART("\r\n'1' : arreter moteur infra");
		envoi_msg_UART("\r\n'2' : robot start");
		envoi_msg_UART("\r\n'3' : robot stop");
		envoi_msg_UART("\r\n'4' : robot turn right");
		envoi_msg_UART("\r\n'5' : robot turn left");
		...
		envoi_msg_UART("->");
		Send_char_SPI(0x30); /* Send 0x30 over SPI to Slave*/
		envoi_msg_UART("\r\n");
	} else if (strcmp((const char *) cmd, "1") == 0) {
		envoi_msg_UART("\r\n");
		envoi_msg_UART((unsigned char *) cmd);
		envoi_msg_UART("->");
		Send_char_SPI(0x31); /* Send 0x31 over SPI to Slave*/
		envoi_msg_UART("\r\n");
